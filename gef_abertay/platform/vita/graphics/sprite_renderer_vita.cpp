...
namespace gef
{
	SpriteRenderer* SpriteRenderer::Create(Platform& platform, IAllocator *alloc)
	{
		return alloc->make<SpriteRendererVita>(platform);
	}
	...
	default_texture_(NULL)
	{
		...
		vertex_buffer_ = gef::VertexBuffer::Create(platform_, IAllocator * alloc);
	...
	void SpriteRendererVita::DrawSprite(const Sprite& sprite, const gef::Matrix33& transform)
	{
		if (shader_ == &default_shader_)
		{
