...
namespace gef
{
	RenderTarget* RenderTarget::Create(const Platform& platform, const Int32 width, const Int32 height, IAllocator *alloc)
	{
		return alloc->make<RenderTargetD3D11>(platform, width, height);
	}
		...
		render_target_view_(NULL)
	{
		...
		TextureD3D11* texture_d3d11 = g_alloc->make<TextureD3D11>(platform_d3d.device_context());
		texture_ = texture_d3d11;
		texture_->setSize({ (float)width, (float)height });
