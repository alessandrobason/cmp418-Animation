...
namespace gef
{
	Texture *Texture::Create(const Platform& platform, const ImageData& image_data, IAllocator *alloc)
	{
		return alloc->make<TextureD3D11>(platform, image_data);
	}
		...
		Texture(platform, image_data)
	{
		...
		CreateTexture(platform, image_data.width(), image_data.height(), image_data.image(), true);
