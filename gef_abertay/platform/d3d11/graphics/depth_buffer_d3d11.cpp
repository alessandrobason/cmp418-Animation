...
namespace gef
{
	DepthBuffer* DepthBuffer::Create(const Platform& platform, UInt32 width, UInt32 height, IAllocator *alloc)
	{
		return alloc->make<DepthBufferD3D11>(
		...
		depth_stencil_shader_resource_(NULL)
	{
		...
		if (SUCCEEDED(hresult) && as_shader_resource)
		{
			...
			if (SUCCEEDED(hresult))
			{
				TextureD3D11* texture_d3d11 = g_alloc->make<TextureD3D11>(device_context);
