...
namespace gef
{
	IndexBuffer* IndexBuffer::Create(Platform& platform, IAllocator *alloc)
	{
		return alloc->make<IndexBufferD3D11>();
	}
	...
	bool IndexBufferD3D11::Init(const Platform& platform, const void* indices, const UInt32 num_indices, const UInt32 index_byte_size, const bool read_only)
	{
		...
		if (!read_only)
		{
			...
			index_data_ = g_alloc->allocDebug(index_byte_size_ * num_indices_, "ibuf index data");
			if (!index_data_)
				success = false;
			else
				if(indices)
