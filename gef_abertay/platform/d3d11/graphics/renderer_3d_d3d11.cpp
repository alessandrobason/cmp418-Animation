...
namespace gef
{
	Renderer3D* Renderer3D::Create(Platform& platform, IAllocator *alloc)
	{
		return alloc->make<Renderer3DD3D11>(platform);
	}
...

	{
		...
		//render_state_desc.ScissorEnable = TRUE;
			...
			if (vertex_buffer && shader_)
			{
				...
				for (UInt32 primitive_index = 0; primitive_index < mesh.num_primitives(); ++primitive_index)
				{
					...
					if (primitive->type() != UNDEFINED && index_buffer)
					{
						...
						int num_indices = (index_buffer && index_buffer->num_indices() > 0) ? index_buffer->num_indices() : vertex_buffer->num_vertices();
						index_buffer->Bind(platform_);
						DrawPrimitive(index_buffer, num_indices);
						index_buffer->Unbind(platform_);
	...
	void Renderer3DD3D11::SetPrimitiveType(gef::PrimitiveType type)
	{
		const PlatformD3D11& platform_d3d = static_cast<const PlatformD3D11&>(platform());

		platform_d3d.device_context()->IASetPrimitiveTopology(primitive_types[type]);
	}

	void Renderer3DD3D11::DrawPrimitive(const IndexBuffer* index_buffer, int num_indices)
	{
		const PlatformD3D11& platform_d3d = static_cast<const PlatformD3D11&>(platform());
		if (index_buffer)
			platform_d3d.device_context()->DrawIndexed(num_indices, 0, 0);
		else
			platform_d3d.device_context()->Draw(num_indices, 0);
	}
