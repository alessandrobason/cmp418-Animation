...
namespace gef
{
	ShaderInterface* ShaderInterface::Create(const Platform& platform, IAllocator *alloc)
	{
		const PlatformD3D11& platform_d3d11 = reinterpret_cast<const PlatformD3D11&>(platform);
		return alloc->make<ShaderInterfaceD3D11>(platform_d3d11.device(), platform_d3d11.device_context());
	}
...
//		CreateSamplerStates();
	...
	void ShaderInterfaceD3D11::CreateVertexFormat()
	{
		...
		elements_ = g_alloc->makeArr<D3D11_INPUT_ELEMENT_DESC>(num_elements_);
