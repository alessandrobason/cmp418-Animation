...
namespace gef
{
	VertexBuffer* VertexBuffer::Create(Platform& platform, IAllocator *alloc)
	{
		return alloc->make<VertexBufferD3D11>();
	}
	...
	bool VertexBufferD3D11::Init(const Platform& platform, void* vertices, const UInt32 num_vertices, const UInt32 vertex_byte_size, const bool read_only)
	{
	...
	bool VertexBufferD3D11::Init(const Platform& platform, const void* vertices, const UInt32 num_vertices, const UInt32 vertex_byte_size, const bool read_only)
	{
		...
		if (!read_only)
		{
			...
			vertex_data_ = g_alloc->allocDebug(vertex_byte_size * num_vertices, "vbuf vertex data");
			if (!vertex_data_)
				success = false;
			else
			{
