...
namespace gef
{
		...
		screenshot_texture_(NULL)
	{
		...
		if(hwnd == NULL)
		{
			window_ = g_alloc->make<WindowWin32>(hinstance, width, height, fullscreen, nullptr);
		...
		window_->setResizeCallback(
			[](UInt32 width, UInt32 height, void *udata) {
				PlatformD3D11 *plat = (PlatformD3D11 *)udata;
				plat->Resize(width, height);
			},
			this
		);
	...
	void PlatformD3D11::Release()
	{
		...
		g_alloc->destroy(window_);

		if (default_texture_)
		{
			RemoveTexture(default_texture_);
			g_alloc->destroy(default_texture_);
	...
	bool PlatformD3D11::InitDeviceAndSwapChain(bool fullscreen)
	{
		...
		if(SUCCEEDED(hresult))
		{
			...
			DXGI_MODE_DESC* display_mode_list = g_alloc->makeArr<DXGI_MODE_DESC>(num_modes);
			...
			g_alloc->destroyArr(display_mode_list);
			display_mode_list = 0;
		}
...
//		debugDev->ReportLiveDeviceObjects(D3D11_RLDO_DETAIL);
	...
	void PlatformD3D11::UpdateFrameTime()
	{
		...
		frame_time_ = (float)cycles / (float)clock_frequency;
	}
	...
	bool PlatformD3D11::ReadyToRender() const 		
	{
	...
	{
		return g_alloc->make<SpriteRendererD3D11>(*this);
	}

	InputManager* PlatformD3D11::CreateInputManager()
	{
		return g_alloc->make<InputManagerD3D11>(*this);
	}

	Renderer3D* PlatformD3D11::CreateRenderer3D()
	{
		return g_alloc->make<Renderer3DD3D11>(*this);
	}
