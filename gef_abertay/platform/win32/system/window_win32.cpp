...
{
	if (ImGui_ImplWin32_WndProcHandler(hwnd, umessage, wparam, lparam) == 1) {
		return 1;
	}
	...
	case WM_SIZE:
		{
			...
			gef::WindowWin32 *win = (gef::WindowWin32 *)ptr;
...
void WindowWin32::setResizeCallback(OnResizeFn callback, void *udata) {
	on_resize_ = callback;
	on_resize_udata_ = udata;
}

void WindowWin32::onResize(UInt32 width, UInt32 height)
{
	width_ = width;
	height_ = height;
	if (on_resize_) {
		on_resize_(width, height, on_resize_udata_);
	}
}
