...
bool Font::Load(const char* font_name)
{
	...
	if(success)
	{
		...
		if(success)
		{
			font_file_data = g_alloc->allocDebug(file_size, "font file data");
	...
	if(success)
	{
		...
		g_alloc->destroy(font_file_data);
		font_file_data = NULL;
...
float Font::GetCharLength(char c) const {
	return (float)character_set.Chars[(UInt32)c].XAdvance;
}
