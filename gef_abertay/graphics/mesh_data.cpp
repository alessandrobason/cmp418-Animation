...
namespace gef
{
	...
	bool MeshData::Read(std::istream& stream)
	{
		...
		{
			PrimitiveData *prim = g_alloc->make<PrimitiveData>();
			prim->Read(stream);
	...
	bool MeshData::Write(std::ostream& stream) const
	{
		...
		for (const auto &prim_iter : primitives) 
	...
	bool VertexData::Read(std::istream& stream)
	{
		...
		vertices = g_alloc->allocDebug(num_vertices*vertex_byte_size, "mesh data vertices");
		if(vertices)
...
//		gef::StringId material_name_id = 0;
		...
		indices = g_alloc->allocDebug(num_indices*index_byte_size, "mesh data indices");
		if(indices)
