...
namespace gef
{
	...
	{
#if 1
		g_alloc->dealloc(vertex_shader_variable_data_);
		g_alloc->dealloc(pixel_shader_variable_data_);
		g_alloc->dealloc(vs_shader_source_);
		g_alloc->dealloc(ps_shader_source_);
#endif
	}
#if 1
	...
	Int32 ShaderInterface::AddVariable(gef::Vec<ShaderVariable>& variables, const char* variable_name, VariableType variable_type, Int32 variable_count)
	{
	...
	void ShaderInterface::SetVariable(gef::Vec<ShaderVariable>& variables, UInt8* variables_data, Int32 variable_index, const void* value, Int32 variable_count)
	{
	...
	UInt8* ShaderInterface::AllocateVariableData(gef::Vec<ShaderVariable>& variables, Int32& variable_data_size)
	{
		variable_data_size = 0;
		for(gef::Vec<ShaderVariable>::iterator shader_variable = variables.begin(); shader_variable != variables.end(); ++shader_variable)
		{
			shader_variable->byte_offset = variable_data_size;
			variable_data_size += GetTypeSize(shader_variable->type)*shader_variable->count;
		}

		return g_alloc->allocDebug<UInt8>(variable_data_size, "shader interface AllocateVariableData");
	}
	...
	void ShaderInterface::SetVertexShaderSource(const char* vs_shader_source, Int32 vs_shader_source_size)
	{
		...
		vs_shader_source_ = g_alloc->allocDebug<char>(vs_shader_source_size, "vs shader source");
	...
	void ShaderInterface::SetPixelShaderSource(const char* ps_shader_source, Int32 ps_shader_source_size)
	{
		...
		ps_shader_source_ = g_alloc->allocDebug<char>(ps_shader_source_size, "ps shader source");
