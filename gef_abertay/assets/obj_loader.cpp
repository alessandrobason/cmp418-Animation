...
bool OBJLoader::Load(const char* filename, Platform& platform, Model& model)
{
	...
	gef::Vec<Texture*> textures;
	gef::Vec<gef::Vector4> positions;
	gef::Vec<gef::Vector4> normals;
	gef::Vec<gef::Vector2> uvs;
	gef::Vec<Int32> face_indices;
	gef::Vec<Int32> primitive_indices;
	gef::Vec<Int32> texture_indices;
	...
	if(success)
	{
		...
		if(success)
		{
			obj_file_data = g_alloc->allocDebug(file_size, "obj file data");
...
//		fclose(file);
		...
		auto vertices = ptr<Mesh::Vertex[]>::make(num_vertices);
		...
		Mesh* mesh = g_alloc->make<Mesh>(platform);
		model.set_mesh(mesh);
		...
		for(gef::Vec<Texture*>::iterator texture=textures.begin(); texture != textures.end(); ++texture)
		{
			Material* material = g_alloc->make<Material>();
		...
		for(UInt32 primitive_num=0;primitive_num<primitive_indices.size();++primitive_num)
		{
			...
			indices[primitive_num] = ptr<UInt32[]>::make(index_count);
...
bool OBJLoader::LoadMaterials(Platform& platform, const char* filename, std::map<std::string, Int32>& materials, gef::Vec<Texture*>& textures)
{
	...
	if(success)
	{
		...
		if(success)
		{
			mtl_file_data = g_alloc->allocDebug(file_size, "mtl file data");
