...
namespace gef
{
    ...
    {
        ptr<File> png_file = gef::File::Create();
        ...
        if(success)
        {
            ...
            //ptr<UInt8[]> buffer = NULL;
            Int32 file_size;
            success = png_file->GetSize(file_size);
            if(success)
            {
                buffer = g_alloc->allocDebug<UInt8>(file_size, "png buffer");
                Int32 bytes_read;
                ...
                if(success)
                {
                    ...
                    if(success)
                    {
						...
						if (colorType == PNG_COLOR_TYPE_RGB_ALPHA)
						{
							...
							UInt8* image_bytes = g_alloc->allocDebug<UInt8>(row_bytes * height, "png image bytes");
            ...
            const void* the_info_ptr, UInt32 width, UInt32 height)
    {
        ...
        ptr<UInt8[]> rowData = ptr<UInt8[]>::make(bytesPerRow);
