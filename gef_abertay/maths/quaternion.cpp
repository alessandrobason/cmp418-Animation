...
namespace gef {
	...
	Quaternion Quaternion::lerp(const Quaternion &startQ, const Quaternion &endQ, float time) {
		Quaternion out;
		out.x = (1.0f - time) * startQ.x + time * endQ.x;
		out.y = (1.0f - time) * startQ.y + time * endQ.y;
		out.z = (1.0f - time) * startQ.z + time * endQ.z;
		out.w = (1.0f - time) * startQ.w + time * endQ.w;
		return out;
	}

	Quaternion Quaternion::slerp(const Quaternion &startQ, const Quaternion &endQ, float time) {
		Quaternion out;
		out.Slerp(startQ, endQ, time);
		return out;
	}
