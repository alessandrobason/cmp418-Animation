...
namespace gef {
	...
	Vector4::Vector4(const Vector2 &pos)
		: Vector4(pos.x, pos.y, 0.f) {}

	Vector4::Vector4(const Vector2 &pos, const Vector2 &size)
		: Vector4(pos.x, pos.y, size.x, size.y) {}
	...
	float Vector4::Angle(const Vector4 &other) const {
		...
		return acosf(gef::clamp(dot, -1.f, 1.f));
	}

	// from unity
	float Vector4::SignedAngle(const Vector4 &other, const Vector4 &axis) const {
		float unsigned_angle = Angle(other);

		gef::Vector4 cross = CrossProduct(other);
		float dot = axis.DotProduct(cross);
		float sign = gef::sign(dot);
