...
namespace gef
{
	...
	const Vector4 TransformAnimNode::GetVector(float _time, const gef::Vec<Vector3Key>& _keys) const
	{
		...
		if(pPrevKey)
		{
			...
			{
				float t = (_time - pPrevKey->time) / (pNextKey->time - pPrevKey->time);
				result = gef::lerp(pPrevKey->value, pNextKey->value, t);
			}
			else
				result = pPrevKey->value;
		}
	...
	Animation::~Animation()
	{
		//for (auto &anim_node : anim_nodes_) 
		//	g_alloc->destroy(anim_node.second);
	}
	...
	{
		PushAllocInfo("Animation3DInit");
		duration_ = animation.duration();
		start_time_ = animation.start_time();
		end_time_ = animation.end_time();
		name_id_ = animation.name_id();
		for (const auto &anim_node_iter : animation.anim_nodes())
		{
			AnimNode* anim_node = NULL;
			switch (anim_node_iter.second->type())
			{
				case AnimNode::kTransform:
					PushAllocInfo("TransformAnimNode");
					anim_node = g_alloc->make<TransformAnimNode>(*((const TransformAnimNode *)(anim_node_iter.second.get())));
					PopAllocInfo();
					break;

				case AnimNode::kChannel:
					PushAllocInfo("ChannelAnimNode");
					anim_node = g_alloc->make<ChannelAnimNode>(*((const ChannelAnimNode *)(anim_node_iter.second.get())));
					PopAllocInfo();
					break;
			}
	...
	void Animation::AddNode(AnimNode* node)
	{
		if(node)
		{
			auto &anim_node_iter=anim_nodes_.find(node->name_id());
	...
	const AnimNode* Animation::FindNode(const StringId name) const
	{
		...
		const auto &anim_node_iter=anim_nodes_.find(name);
		if(anim_node_iter != anim_nodes_.end())
			result = anim_node_iter->second.get();
	...
	void Animation::CalculateDuration()
	{
		...
		{
			duration_ = 0.0f;
			for(auto &anim_node_iter : anim_nodes_)
			{
	...
	bool Animation::Read(std::istream& stream)
	{
		...
		for(Int32 anim_node_num=0; anim_node_num < num_anim_nodes; ++anim_node_num)
		{
			...
			{
			case AnimNode::kTransform:
				anim_node = g_alloc->make<TransformAnimNode>();
				break;

			case AnimNode::kChannel:
				anim_node = g_alloc->make<ChannelAnimNode>();
				break;
			}
	...
	bool Animation::Write(std::ostream& stream) const
	{
		...
		for(const auto &anim_node_iter : anim_nodes_)
		{
	...
	Animation &Animation::operator=(Animation &&anim) {
		std::swap(duration_, anim.duration_);
		std::swap(start_time_, anim.start_time_);
		std::swap(end_time_, anim.end_time_);
		std::swap(name_id_, anim.name_id_);
		std::swap(anim_nodes_, anim.anim_nodes_);
		return *this;
	}
